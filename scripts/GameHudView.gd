...
static func create_runtime_ui(owner, game_layer: Node, battlefield, current_layout: Dictionary, view_size: Vector2, mobile_mode: bool) -> Dictionary:
    ...
    var top_panel = UIFactory.make_panel_shell(Vector2((view_size.x - top_panel_w) * 0.5, 8), Vector2(top_panel_w, top_panel_h), Color(0.93, 0.96, 1.0, 0.98))
    ...
    var leader_label = UIFactory.make_outline_label(Vector2(16, 4), Vector2(176, 24), "", 14 if mobile_mode else 15, Color.WHITE, HORIZONTAL_ALIGNMENT_LEFT, 2)
    top_panel.add_child(leader_label)

    var timer_label = UIFactory.make_outline_label(Vector2((top_panel.size.x - 110.0) * 0.5, 1), Vector2(110, 26), "00:00", 18 if mobile_mode else 20, Color(1.0, 0.96, 0.72), HORIZONTAL_ALIGNMENT_CENTER, 3)
    top_panel.add_child(timer_label)

    var stage_label = UIFactory.make_outline_label(Vector2(top_panel.size.x - 182.0, 4), Vector2(168, 24), "", 13 if mobile_mode else 15, Color(0.82, 0.92, 1.0), HORIZONTAL_ALIGNMENT_RIGHT, 2)
    ...
    var game_title_label = UIFactory.make_outline_label(Vector2(0, 64), Vector2(top_panel.size.x, 28), "领土战争", 24 if mobile_mode else current_layout.get("title_font", 31), Color(1.0, 0.95, 0.72), HORIZONTAL_ALIGNMENT_CENTER, 5)
    ...
    var pause_overlay = _create_pause_overlay(owner, view_size)
...
static func _create_pause_overlay(owner, view_size: Vector2) -> Control:
    ...
    var pause_panel = UIFactory.make_panel_shell(Vector2((view_size.x - 260.0) * 0.5, (view_size.y - 140.0) * 0.5), Vector2(260, 140), Color(0.94, 0.97, 1.0, 0.96))
