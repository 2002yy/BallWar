...
- Theme / 主题:
  ...
  - decor layer event-driven + ChamberState extraction

Key points / 重点:

- PlayerSettingsStore: persistent settings (perf/log/low-effect), OS.is_debug_build() default
- SettingsPanel: 3 working CheckButtons, live toggle via settings_changed signal
- ResultPanel: end-of-match screen with replay/return, match_finished flag prevents re-continue
- BulletPool.peak_active_count + EventRouletteController.event_count statistics
- BattlefieldDecorLayer: dirty-flag pattern replaces per-frame polling
- ChamberState.gd: pure state container extracted from ControlChamber

Latest recorded baseline / 最近记录的基线:

- active correctness baseline: 886 checks across 8 test suites
- latest local headless pass snapshot is recorded in `AI_HANDOFF_CURRENT.md`
- detailed stage writeup in `README_v2_1_9_settings_and_result_panel.md`
